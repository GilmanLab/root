...
| **P2 — Adopt** | Run `init`, `snapshot`, and the adoption plan. The plan may contain imports, in-place updates, and adopt-by-create rows, but no replacements. Import the existing bridge, five bridge-port rows, VLAN 10 and 40 rows, `mgmt-vlan10`, static address, and default route. Service rows appear as creates because the provider's `routeros_ip_service` import is defective (its importer stores the internal ID that its name-keyed read cannot resolve); RouterOS applies those creates as `set` on the existing services. The `tofu-svc` group is never imported or managed: `svc-tofu` deliberately lacks the `policy` permission, so OpenTofu cannot modify user groups, and groups and users stay runbook-owned. Remove `imports.tf` after the successful adoption apply. Record the saved-plan summary and empty follow-up plan. |
