...
| T41 | Create `GilmanLab/sandbox` (pyinfra base config for `sandbox01`). **Plan drafted: `.journal/002/SANDBOX_SETUP_PLAN.md`** — admin group/users, Docker+Podman, Incus via pyinfra-incus, Tailscale + Tailscale-SSH primary (networking policy PR: `tag:sandbox` + SSH rules; first `sandbox` secrets scope for the auth key), SSH hardening last, baseline; acceptance incl. idempotence + both auth paths | blocked | Handed to implementation session; closes on report. Pyinfra confirmed as house tool (also drives VyOS) |
