...
| T27 | Talos image pipeline: builder TBD + CAPI integration research. Key insight to verify: CAPN injects machine config at instance create via cloud-init disk, so images may stay generic (no per-node seeding); evaluate self-hosting Sidero image-factory vs. bespoke | open | Research before building |
