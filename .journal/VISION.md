...
- Storage network: non-routed VLAN 30 (`10.10.30.0/24`) on the fast links —
  lab SFP+ pairs as active-backup bonds (LACP dropped in session 010: the
  hosts never received a switch LACPDU, itself a symptom of T48), nas01
  10GbE on port 7. Host side converged and links up at 10G; datapath still
  blocked by T48 after the 10G DAC swap (session 013). Incus cluster
...
| T25 | IncusOS image tooling → product-side: `componere/incusos-builder` **done** (v0.1.0, init/validate/build/versions, native SOPS in-memory decrypt, full manual); release process in a separate stream — lab builds from source until then | resolved-for-lab | Consumed via local source build; re-pin on first release |
...
| T48 | ice Safe Mode (no DDP firmware) blocks VLAN 30 into the lab nodes (was misattributed to RouterOS, then to FEC alone) | open | Session 013 retest after Josh swapped all six 25G DACs for 10G: the FEC leg is CLOSED (links now `SFP-H10GB-CU3M`, 10G, `Requested FEC: NONE, Negotiated FEC: NONE`; previously RS-FEC requested / NONE negotiated), but the datapath is still dead switch→lab. Characterized precisely: in Safe Mode the E810-XXV passes broadcast/multicast ONLY — it drops all unicast, including to its own primary MAC (`promisc on` does not help), and drops every VLAN-tagged frame regardless of type. Measured with switch counters (port tx +251,185 B while host rx stayed flat), container tcpdump, and split unicast/broadcast AF_PACKET ARP probes (0/3 unicast answered, 3/3 broadcast answered in ~300us). CONSEQUENCE: an untagged VLAN 30 is NOT a viable stopgap; nothing at the switch or VLAN layer can help. Host+switch config is correct and converged (fleet#4, networking#15; storage 18/18 and network 4/4 no-change). SOLE remaining blocker: the stable release carrying lxc/incus-os#1306 (merged 2026-08-22T05:04Z, `de271730`) — newest published build is still `202608201218`, which all four nodes run; `auto_reboot: false`, so it needs a manual reboot. Retest: two containers with macvlan on fast30 + cross-node ping, then storage/network deploy reruns. Fallback if it becomes urgent: local `incusos-builder` image with the fix + reinstall the three labs (deferred, Josh's call) |
| T49 | Promote fleet_cluster ops upstream to meigma/pyinfra-incus | resolved | Session 011 filed three issues; upstream shipped all three within two days, and fleet now builds **no** incus commands of its own. [#20](https://github.com/meigma/pyinfra-incus/issues/20) → 0.2.1 named remotes + 0.2.2 IncusOS `/os/1.0` facts/ops; [#21](https://github.com/meigma/pyinfra-incus/issues/21) → 0.2.3 `cluster_certificate` + public `certificate_fingerprint`; [#38](https://github.com/meigma/pyinfra-incus/issues/38) → 0.2.4 `cluster_storage_pool` + `StoragePoolMember`. Consumed in fleet#6 (certificate, local impl deleted) and fleet#7 (storage/network migrated, `_cli.py` + `facts.py` deleted, −824 lines; 18/18 operations dry-run no-op against the live cluster). What deliberately stays lab-local: the node inventory, the `fast`/`fast30` desired-document builder with its `mgmt` guard, the TLS SAN list, and read-only assertions for recovery-key escrow and `scrub_schedule` (upstream preserves but does not manage it — raised on #38, not filed). Both PRs await merge; #7 is stacked on #6 |
