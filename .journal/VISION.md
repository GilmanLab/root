...
| T48 | E810 PHY-level RX blackhole blocks VLAN 30 into the lab nodes (was misattributed to RouterOS) | open | Switch→host frames never reach the E810 MACs (rx_bytes 0, ports promisc) while host→switch works — killed LACP (hosts never received LACPDUs, mux stayed Defaulted) and kills all fast-path ingress. Kernel log: ice "Entering Safe Mode" (IncusOS ships NO firmware — intel/ice/ddp/ice.pkg absent from mkosi base image) + "Requested FEC: RS-FEC, Negotiated FEC: NONE" on 25G DACs forced to 10G. RouterOS exonerated (identical on 7.16.2 and 7.24; upgrade to 7.24+fw kept). Current state: hosts active-backup + vlan_tags (fleet#4), switch plain tagged ports 1–7 (networking#15) — correct config, blocked on: (a) file lxc/incus-os issue to ship the ice DDP (fixes Safe Mode; likely fixes FEC policy), and/or (b) swap the six 25G DACs for 10G-rated media (nas01's port-7 10G optic is the proven pattern). Retest: two containers with macvlan on fast30 + ping, then storage/network deploys re-run |
