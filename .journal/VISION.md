...
| T49 | Promote fleet_cluster ops upstream to meigma/pyinfra-incus | open | Lab-proven gaps per T26 flow: cluster `--target` storage_pool (pending-per-member + final create), /os/1.0 system storage/network/security facts+ops (full-replace PUT + confirmation-timeout confirm), literal `--data` payload handling (no stdin form for `incus query`). Session 011 audit (fleet#6) + **filed upstream**: [meigma/pyinfra-incus#20](https://github.com/meigma/pyinfra-incus/issues/20) IncusOS compatibility — upstream `incus_command` (`_cli.py:75`) has no remote concept and the README model needs shell+sudo on the target, which IncusOS cannot provide; asks for opt-in `remote=` (verified `incus query myremote:/1.0` and `INCUS_REMOTE=` both work, composing with existing `?target=`), and notes the `incus query` argv-payload carve-out + `/os/1.0` as follow-up. [#21](https://github.com/meigma/pyinfra-incus/issues/21) cluster certificate — upstream covers the trust store (`operations.trusted_certificate`) but nothing for `cluster.crt`; asks for a `cluster_certificate` operation (fingerprint diff vs `environment.certificate_fingerprint`, all-members-Online precondition, no-rollback docs, file-path key contract) and for making `_control_plane.certificate_fingerprint` public. Once both land, fleet deletes its `_cli.py` and duplicated `Server`/`ClusterMembers`/`StoragePools` facts |
