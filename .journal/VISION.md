...
| T49 | Promote fleet_cluster ops upstream to meigma/pyinfra-incus | open | Lab-proven gaps per T26 flow: cluster `--target` storage_pool (pending-per-member + final create), /os/1.0 system storage/network/security facts+ops (full-replace PUT + confirmation-timeout confirm), literal `--data` payload handling (no stdin form for `incus query`). Added by session 011 (fleet#6): (1) **cluster certificate** — upstream has `operations.trusted_certificate` (trust store) but nothing for `incus cluster update-certificate`, so `cluster.crt` replacement has no upstream home; (2) expose `certificate_fingerprint` publicly — it already exists at `_control_plane.certificate_fingerprint` (stdlib `ssl.PEM_cert_to_DER_cert` + private-key guard) but is private, so fleet re-implements it; (3) **remote support** is the real blocker for reusing ANY upstream fact/op here — upstream `incus_command` builds bare `incus …` for a local socket over the host's connector, while IncusOS has no SSH so fleet runs `incus nas01:…` from the operator box. Verified `INCUS_REMOTE=nas01 incus query /1.0` works, so a `remote=`/env option upstream would let fleet delete its `_cli.py` and its duplicated `Server`/`ClusterMembers`/`StoragePools` facts |
