...
| T48 | E810 PHY-level RX blackhole blocks VLAN 30 into the lab nodes (was misattributed to RouterOS) | open | Switch→host frames never reach the E810 MACs (rx_bytes 0, ports promisc) while host→switch works — killed LACP (hosts never received LACPDUs, mux stayed Defaulted) and kills all fast-path ingress. Kernel log: ice "Entering Safe Mode" (IncusOS ships NO firmware — intel/ice/ddp/ice.pkg absent from mkosi base image) + "Requested FEC: RS-FEC, Negotiated FEC: NONE" on 25G DACs forced to 10G. RouterOS exonerated (identical on 7.16.2 and 7.24; upgrade to 7.24+fw kept). Current state: hosts active-backup + vlan_tags (fleet#4), switch plain tagged ports 1–7 (networking#15) — correct config, blocked on: (a) UPSTREAM FIX MERGED — lxc/incus-os#1306 (app-build route, per stgraber; issue #1305): waiting on the next IncusOS release, lands via stable-channel update; (b) 6x 10G SFP+ DACs ON ORDER, ETA ~2026-08-24 — swap the six lab links (10G-rated media negotiates no FEC even in Safe Mode; nas01's port-7 optic is the proven pattern). Retest: two containers with macvlan on fast30 + ping, then storage/network deploys re-run |
| T49 | Promote fleet_cluster ops upstream to meigma/pyinfra-incus | in-progress | Session 011 filed three issues and upstream shipped two of them within a day. **Done**: [#20](https://github.com/meigma/pyinfra-incus/issues/20) (closed) → 0.2.1 named remotes (`remote=` on every fact/op) + 0.2.2 IncusOS `/os/1.0` facts `IncusOS{Storage,Network,Security,Update,Provider}` and ops `incus_os_{storage_pool,storage_volume,network,update,provider}`; [#21](https://github.com/meigma/pyinfra-incus/issues/21) (closed) → 0.2.3 `operations.cluster_certificate` + public `certificate_fingerprint`, now consumed by fleet (fleet#6: local implementation deleted, key custody in `fleet_cluster/tls.py`). **Open**: [#38](https://github.com/meigma/pyinfra-incus/issues/38) cluster-scoped storage — `storage_pool`/`storage_volume` and `StoragePools`/`StorageVolumes` have no `target=`, so the two-step per-member define + instantiate flow and the member-only `config.source` view (live proof: cluster GET shows `config:{}`, `?target=lab01` shows `source=data/incus`) cannot be expressed. That is the last thing keeping fleet's `_cli.py`, `IncusStoragePools`, `IncusStoragePoolMember` and `cluster_storage_pool` local. Remaining follow-up once #38 lands: migrate the storage/network deploys onto the `incus_os_*` operations (their semantics differ — fleet refuses reshape rather than offering present/absent, and guards `mgmt` before a full-replace network PUT), then delete `_cli.py` |
