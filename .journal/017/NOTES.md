...
## 2026-08-26 22:12 — Bootstrap ordering question resolved
Josh asked whether root SPIRE on the platform cluster is chicken-and-egg with its own Talos nodes. No: acyclic ordering (Talos machine CA forms the cluster identity-free → server pod → agents retry-attest; can_reattest, pull-based). Two invariants to preserve: nothing below SPIRE requires SPIRE (registry, CSI, CNI, Talos PKI stay SVID-free); agent bootstrap trust = bundle file in GitOps (one-time export after first server boot), not a live service. Real design item surfaced: mgmt-cluster REBUILD posture — restore datastore+CA keys (same bundle, seamless) vs new root (lab-wide bundle rotation); must land in the bootstrap design beside re-pivot recovery; cattle intermediate ca_ttl = rebuild time budget.

## 2026-08-26 22:26 — CA hierarchy discussion
Grounded in aws/security/pki/root-ca: offline-by-policy KMS root already exists (ECC P384, non-exportable, 20y, alias glab-pki-root-ca, cert+fingerprint in git, step+step-kms-plugin ceremony documented). Evaluated Josh's four options: KMS root ✔ (already built; S3 unnecessary, cert in git); root-in-Vault ✗ (root must outlive mgmt cluster); sub-CAs under external root ✔ shape — but SIBLINGS not the README's sketched stack; step-ca on the Keycloak instance ✗ (ceremony needs no server — step CLI against KMS IS smallstep; online issuance = Vault PKI + Let's Encrypt DNS-01; T38 tears the instance down).
FINDING: committed root is pathlen:2, minted for the sketched Root→Vault→SPIRE→leaf chain. That sketch (a) predates nested-B — nested needs 3 intermediates (upstream int → root server CA → downstream CA), invalid under pathlen:2 — and (b) stacks SPIRE under Vault, reinstating the rebuild-cycle risk. Recommended: re-mint root cert NOW (same KMS key, pathlen:3 or none, ~zero consumers today); Vault + SPIRE as sibling intermediates; SPIRE via UpstreamAuthority disk with SOPS-custodied intermediate (also solves rebuild = same bundle, no rotation). Caveat noted: disk intermediate = exportable signing key in SOPS (accepted custody family). Open decisions: root re-mint y/n; SPIRE chained vs self-contained; step-ca stays dead.

## 2026-08-26 22:44 — CA hierarchy documented; both PRs merged
Josh accepted the sibling-intermediate shape. Documented per gilmanlab-documentation skill:
- ADR-0005 "Anchor Internal PKI on the KMS Root with Sibling Intermediates" merged to GilmanLab/root (root#28, squash bad64c7c): root cert re-mint (same KMS key, NO pathlen; depth bounded per-intermediate: Vault pathlen:1, SPIRE upstream pathlen:2), SPIRE via disk UpstreamAuthority with SOPS-custodied key, no standing CA, public ACME for browser-facing, Vault never upstream of SPIRE. Strict docs build green.
- GilmanLab/aws aws#5 merged (squash ccf86dff): root-ca README hierarchy corrected (sibling diagram + pending-re-mint note + ADR link), root-ca.tpl maxPathLen removed.
PENDING OPERATOR ACTION: re-mint root_ca.crt with the updated template at next ceremony (aws sso + step-kms-plugin; fingerprint rotates — do before intermediates are issued).
PROMOTE TO TECH_NOTES AT CLOSE: internal PKI hierarchy is decided per ADR-0005; root-ca README is current; re-mint pending.

## 2026-08-26 22:58 — Mgmt-cluster state-of-decisions survey
Searched VISION.md (compute platform, bootstrap sequence, full tracker), 016 spike record, 002 plans, central docs. Decided: 3-node Talos on nas01 (risk accepted, T21 stretch deferred); services Zitadel/Vault/CAPI; CAPI self-managed pivot (T32 + working manifest); generic nocloud images cold-starting from public factory (T27, T31 later); bridged VLANs (T11); nas01 HTTPS listener + CAPN trust via fleet pyinfra; steps 1–2 done, step 3 is next. Open on critical path: T08 (template — now unblocked, undesigned; kube-vip swap recorded), T10 (kube-vip L2 vs gw01 no-BGP ruling — mgmt cluster is the first consumer), T22 (GitOps engine + GitHub-hard-dependency ruling), T24 (Vault bootstrap/DR), step-4 mechanism STILL UNDUMPED (Josh's head). Silent gaps found: no StorageClass/CSI decision (Zitadel/Vault/SPIRE PVCs; spike flagged too), no VLAN/address/VIP plan rows for mgmt VMs, no repo home for cluster template/GitOps config (T26 "later repos" unnamed), SPIRE trust-domain name + root re-mint pending, zero central-docs coverage (T14's bootstrap design draft never written). Proposed: dump step-4 idea → T08-minimal → execute pivot, design doc emerges after.

## 2026-08-26 23:05 — Decision walk 1/8: mgmt-VM network placement
DECIDED (Josh): dedicated platform VLAN for the mgmt cluster (option B) — not VLAN 10 (crowds infra L2/subnet with k8s nodes+VIPs) and not VLAN 40 (sandbox neighborhood). Physically viable today via nas01's proven 10G fast link tagged to sw-core01 (T48 irrelevant: VMs are nas01-only). Implementation = bounded change across fleet vlan_tags + sw-core01 tofu + gw01 template. Proposed concretes (Josh to veto at implementation): VLAN 50, 10.10.50.0/24, gw .1, nodes .11–.13, kube-vip VIP .10, LB range reserved. Rationale: placement is rebuild-grade to change later; sets the T10 pattern for cattle supernets.
